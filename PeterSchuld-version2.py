import csv
costcoFile=open('costco-citi-visa-due-soon - bob copy - sorted - with sparse categories.csv')
...
for i in range(n-1):
    ...
    category=costcoData[i+1][5]
    if category=='':
        costcoData[i+1][5]=mapping.get(Vendor,'WARNING-NO-CATEGORY')
    else:
        continue
...
outputFile=open('output.version-2.budget-monthly-totals-and-tracking.csv','w')
